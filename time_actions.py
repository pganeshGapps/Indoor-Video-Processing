...
######### BACKGROUND SUBTRACTION
	...
	for c in cnts:
		...
		maps[(x,y,x+w,y+h)] = False
		#cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)
	final = []
	for (x1,y1,x2,y2) in rects2 :
		(a,b) = ((x1+x2)/2,y1)
		if maps[(x1,y1,x2,y2)] == False :
			maps[(x1,y1,x2,y2)] = True
			arr = []
			arr.append((x1,y1,x2,y2))
			for (x3,y3,x4,y4) in rects2 :
				(c,d) = ((x3+x4)/2,(y3+y4)/2)
				if sqrt((a-c)*(a-c)+(b-d)*(b-d)) < 100 and maps[(x3,y3,x4,y4)] == False :
					arr.append((x3,y3,x4,y4))
					maps[(x3,y3,x4,y4)] = True
			final.append(arr)
	pick2 = []
	for arr in final :
		minx = 10000000
		...
		for (x1,y1,x2,y2) in arr :
			...
			if maxy!= None and maxy < y2 :
				maxy = y2
		pick2.append((minx,miny,maxx,maxy))
