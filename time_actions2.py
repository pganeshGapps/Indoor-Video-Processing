...
############ BACKGROUND SUBTRACTION
	...
	if time.time() > ticks + starttime and time.time() < ticks + starttime + timelimit :
		...
		cv2.putText(frame, "{}".format("ALERT - "+str(float(text3)/10)), (10,20),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
		#os.system('say "beep"')
	#cv2.putText(frame, "{}".format(text1), (frame.shape[0]/2+50, 20),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
	if disp :
	...
	cv2.putText(frame, datetime.datetime.now().strftime("%A %d %B %Y "),	(10, frame.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 0, 255), 1)
	if counter2 >=0 and counter2 <=50 :
		counter2 = counter2-1
		cv2.rectangle(frame,(300,100),(400,250),(0,255,0),2)
	elif counter2 >50:
		counter2 = counter2 -1
	else :
		counter2 = random.randint(1,100)
...
for k in path :
	...
	for (x1,y1) in path[k] :
		...
		if miny!= None and miny > y1 :
			miny = y1
		if maxx!= None and maxx < x1 :
			maxx = x1
		if maxy!= None and maxy < y1 :
			maxy = y1
	avgminx = avgminx + minx*len(path[k])
	avgmaxx = avgmaxx + maxx*len(path[k])
	avgminy = avgminy + miny*len(path[k])
	avgmaxy = avgmaxy + maxy*len(path[k])
	sum_path = sum_path + len(path[k])
avgminx = (avgminx/sum_path)
avgminy = (avgminy/sum_path)
avgmaxx = (avgmaxx/sum_path)
avgmaxy = (avgmaxy/sum_path)
cv2.rectangle(lastframe,(avgminx,avgminy),(avgmaxx,avgmaxy),(0,255,0),2)
cv2.imshow("Human Tracking",lastframe)
while (cv2.waitKey(1) & 0xFF) != ord("q") :
	#cv2.rectangle(frame,(117,157),(166,140),(255,255,255),2)
	print "press q - " + str(avgminx)+" "+str(avgminy)+" "+str(avgmaxx)+" "+str(avgmaxy)
